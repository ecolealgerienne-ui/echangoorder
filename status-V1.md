...
| F02 | Authentification (inscription/connexion/invité) | Terminé | En cours — endpoints validés en réel | ☐ | Backend Odoo `/echango/auth/register` + `/login` testés avec succès contre un Odoo 19 réel (voir §0). Client Flutter (JSON-RPC) pas encore branché sur ces endpoints — prochaine étape |
| F03 | Accueil | Terminé | ☐ | ☐ | |
