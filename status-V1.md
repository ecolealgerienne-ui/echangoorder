...
**🏷️ Renommage de marque "Echango Market" (décision produit, 2026-07-23)** : nom affiché côté utilisateur changé partout (`common.appName`, titre `MaterialApp`, pages légales CGU/confidentialité/mentions légales/cookies, textes permission GPS et partage produit — FR/AR) + `mobile/pubspec.yaml` (nom du package Dart) — décidé avant toute publication sur les stores pour fixer le bundle ID (`com.echangomarket`, voir CLAUDE.md § Contexte projet) sans casser une app déjà publiée. Identifiants techniques internes inchangés : module Odoo `echango_order`, repo GitHub, noms de branches — "Echango Order" reste le nom de code interne du projet dans cette documentation. **Non testé en réel** (nécessite de régénérer `android/`/`ios/` ou de corriger l'`applicationId`/bundle identifier s'ils existent déjà en local sous l'ancien org).

**🐛 Bug signalé par l'utilisateur, corrigé en 3 passes (2026-07-23)** : `CartBar` continuait d'afficher/de masquer incorrectement le panier autour d'une commande confirmée.
- **1re passe** (`CartState.clearLocally()` juste après confirmation) : insuffisante, le bug revenait dès qu'un vrai `refresh()` ultérieur (ex. réouverture de l'écran Panier) retrouvait la même commande, toujours "le panier courant" côté serveur (`state='sent'` inclus dans le domaine de `cart_controller.py._cart_order()`).
- **2e passe** (exposer `state`, `CartState.hasActiveCart` basé sur `state == 'draft'`) : corrigeait l'affichage, mais révélait un effet de bord plus grave — tout nouvel ajout au panier s'attachait aussi silencieusement à cette même commande déjà confirmée (toujours dans le domaine `draft`/`sent`), rendant le panier invisible pendant potentiellement des heures en conditions réelles (tant qu'un opérateur n'a pas pris en charge la commande).
- **3e passe, décision produit "rupture nette"** : `cart_controller.py._cart_order()`/`_owned_line()` ne considèrent plus qu'une commande `draft` comme panier courant (`sent` retiré de ces deux domaines) — un ajout après confirmation démarre systématiquement une commande brouillon neuve. `checkout_controller.py.confirm()` garde son propre domaine `draft`/`sent` inchangé (modifier adresse/créneau avant prise en charge reste possible, sujet différent). **Non testé en réel.**
