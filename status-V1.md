...
| F03 — Accueil : affichage produits (Flutter + backend) | Terminé (code), non exécuté | **Backend** : `security/ir.model.access.csv` + `security/ir_rule.xml` dans `echango_order` — le groupe portail n'avait par défaut AUCUN accès à `product.template`/`product.category` (vérifié contre le code source Odoo 19 des modules `product` et `sale`, aucune entrée portail trouvée), donc accès lecture seule accordé + `ir.rule` restreignant aux produits `sale_ok = true` (pas de fuite du catalogue interne). **Flutter** : `HomeScreen` réécrit — `OdooApiClient.searchRead()` (nouveau, `search_read` standard via `call_kw`, pas de contrôleur custom) sur `product.template` (`name`, `list_price`, `image_128`), grille 2 colonnes, état vide (`emptyStates.products*`, nouvelles clés FR/AR ajoutées) et état d'erreur via `ErrorStateView` existant. **Écart assumé vs spec Expert Odoo** : filtre `sale_ok = true` à la place de `is_published` (champ qui n'existe pas sans le module `website_sale`, non installé) — pas de notion de "produits mis en avant" pour l'instant, tous les produits vendables s'affichent ; la curation viendra avec `x_vitrine_publique` (F00) si besoin. Nécessite mise à jour du module (`-u echango_order`, nouveaux fichiers de sécurité) avant test |
...
| F03 | Accueil | Terminé | Terminé (code) — grille produits sur `product.template`, non testé en réel | ☐ | Voir §0 pour le détail (accès portail accordé, filtre `sale_ok`, pas encore de "produits mis en avant"/bannière/catégories) |
