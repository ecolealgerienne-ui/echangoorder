...
**🐛 Bug signalé par l'utilisateur, corrigé en 2 passes (2026-07-23)** : `CartBar` continuait d'afficher le total d'une commande déjà confirmée. Pas un résidu — le devis reste volontairement "le panier courant" côté serveur (`state='sent'`, domaine `draft`/`sent` de `cart_controller.py._cart_order()`) pour permettre d'y ajouter des articles tant qu'un opérateur ne l'a pas pris en charge (CLAUDE.md § Statuts de commande). **1re passe** (`CartState.clearLocally()` à la place de `refresh()` juste après confirmation) insuffisante : le bug revenait dès qu'un vrai `refresh()` ultérieur (ex. réouverture de l'écran Panier) re-décrivait fidèlement cette même commande toujours "courante" côté serveur. **2e passe, après clarification produit** (garder la possibilité d'ajouter après confirmation, mais ne plus jamais l'afficher comme panier) : `_cart_payload()` expose désormais `state` ; nouveau `CartState.isDraft`/`hasActiveCart` (`state == 'draft'` ET des lignes) ; `CartBar` se fie à `hasActiveCart` plutôt qu'à `itemCount` seul — reste masquée même après un `refresh()` qui retrouve la commande confirmée, puisque son `state` n'est plus `'draft'`. **Non testé en réel.**
