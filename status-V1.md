...
| Client API JSON-RPC Odoo (Flutter) | Terminé (F02) | `lib/services/odoo_api_client.dart` (`register`/`login`, appelle `/echango/auth/*`, cookie de session Odoo gardé en mémoire) + `lib/config/api_config.dart` (URL de base dev, `10.0.2.2` sur émulateur Android). Branché sur `LoginScreen` et `RegisterStep3Screen` (le téléphone saisi à l'étape 1 transite via `extra` de go_router jusqu'à l'étape 3 pour l'appel `register`). Toutes les constantes `AppError` nécessaires existaient déjà (`authInvalidCredentials`, `authPhoneAlreadyUsed`, `authPinLocked`, `validationRequired`, `validationInvalidPin`) — mappage fait dans `odoo_api_client.dart`, aucune traduction à ajouter. **Non testé** (pas de SDK Flutter dans ce sandbox) — à vérifier côté utilisateur : `flutter pub get` (nouvelle dépendance `http`), `flutter analyze`, puis test réel inscription/connexion sur émulateur contre le backend WSL. **Limite connue** : cookie de session gardé en mémoire seulement (pas persisté) — perdu au redémarrage de l'app ; le stockage sécurisé (`flutter_secure_storage`, voir ligne juste en dessous) reste à faire avant une vraie gestion de session 24h |
...
| F02 | Authentification (inscription/connexion/invité) | Terminé | En cours — backend validé en réel, client Flutter écrit non testé | ☐ | Backend Odoo `/echango/auth/register` + `/login` testés avec succès contre un Odoo 19 réel (voir §0). Client Flutter écrit et branché sur Login/Register (voir §0, ligne "Client API JSON-RPC Odoo") — reste à valider avec `flutter run` côté utilisateur. Mode invité déjà géré localement (pas d'appel Odoo requis) |
| F03 | Accueil | Terminé | ☐ | ☐ | |
