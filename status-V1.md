...
| F04 — Catalogue & Recherche (Flutter + backend) | Terminé (code), non testé en réel | Aucun changement backend nécessaire (mêmes droits portail que F03, `product.template`/`product.category` déjà ouverts). **Flutter** : `CatalogScreen` (liste des catégories, `product.category` `search_read`), `CategoryProductsScreen` (grille filtrée `categ_id`, nom de catégorie transmis via `extra` de go_router depuis `CatalogScreen` pour éviter une requête supplémentaire), `SearchScreen` (`name ilike`, anti-rebond 400ms via `Timer`, pas de nouvelle dépendance). Nouveau widget partagé `lib/widgets/product_grid_tile.dart` (`ProductGridTile`) utilisé par Accueil/Catalogue/Recherche — évite de dupliquer 3 fois le décodage d'image + la mise en page ; ajoute un badge "Épuisé" (`qty_available <= 0`, champ pas demandé sur l'Accueil) et un bouton "+" optionnel (absent sur l'Accueil, présent en Catalogue/Recherche comme dans le wireframe). Le "+" appelle `showComingSoon` — l'ajout réel au panier est F06, pas encore implémenté. Nouvelles clés `catalog.outOfStock` + `emptyStates.categories{Title,Message}` en FR/AR. **Non fait dans cette passe** : barre de recherche accessible depuis l'Accueil (QA F03, jamais implémentée, risque de complications de navigation cross-onglet avec `StatefulShellRoute` — à traiter séparément si besoin) |
