...
5. **Prête** — le bon de livraison est validé (`stock.picking.state == 'done'`) — `order_controller.py._prep_status()` renvoie `completed`. Pour un retrait en magasin, cette validation **est** la remise au client (pas de notion de statut de livraison distinct, voir ci-dessous).
