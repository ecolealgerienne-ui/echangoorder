...
- **Modèle Odoo retenu — standard avant custom** : route de livraison **Pick + Pack + Ship** (3 étapes), fonctionnalité standard du module `stock` (Community, confirmé — pas besoin d'Enterprise). Pick (Stock → zone de tri) = la collecte, regroupable via `stock.picking.batch` (standard) sur plusieurs commandes. Pack (zone de tri → zone de sortie) = le poste physique demandé (contrôle qualité + séparation par commande) — usage conforme à la fonction native de cette étape, pas un détournement. Ship (zone de sortie → client/comptoir) = dernier maillon inchangé, reste `picking_type_id.code == "outgoing"`. **Activation de la route 3 étapes = configuration manuelle back-office** (Réglages > Inventaire > activer "Multi-Step Routes", puis fiche Entrepôt > Expéditions sortantes > "Réceptionner en zone de tri, puis expédier (3 étapes)") — pas de code, comme pour `x_delivery_zone` (config métier, pas une décision technique figée dans le module).
- **Bug trouvé au 2e test réel (2026-07-22)** : `NotNullViolation` sur `order_id` en sauvegardant le wizard — les champs `readonly="1"` de la liste (`order_id`, `line_count`, `qty_total`), peuplés seulement via `default_get`/`_compute_suggestions()` (pas un vrai `compute`), n'étaient pas renvoyés au serveur par le client web à la création des lignes (comportement standard du client web Odoo depuis la v11 pour les champs readonly non calculés). Corrigé par `force_save="1"` sur ces 3 champs (`views/batch_picking_wizard_views.xml`) — mécanisme standard prévu pour exactement ce cas.
- **Bug trouvé au premier test réel (2026-07-22)** : `AttributeError: 'stock.picking' object has no attribute 'batch_id'` à l'ouverture du wizard — `stock.picking.batch_id` n'est pas défini dans le module `stock` de base (contrairement à l'hypothèse initiale de la revue Odoo), mais dans le module séparé `stock_picking_batch` (Community, LGPL-3, dépend uniquement de `stock`), jamais déclaré en dépendance du module. Corrigé (`__manifest__.py`, `depends`) — nécessite `-u echango_order`, Odoo installe la dépendance manquante automatiquement.
- **Correctif trouvé en revue (avant codage) et appliqué** : `order_controller.py._prep_status()` filtrait uniquement le picking `outgoing` — sur une route à 3 étapes, ce picking reste `waiting`/`confirmed` (jamais `assigned`) tant que le Pack n'est pas validé, ce qui aurait bloqué le statut app sur "pending" pendant toute la collecte et le tri. Corrigé : `_prep_status()` regarde désormais tous les pickings actifs de la commande — "in_progress" se déclenche dès qu'un opérateur s'assigne N'IMPORTE QUEL picking de la chaîne (Pick, Pack ou Ship), pas besoin d'attendre le tri. Décision produit : pas de nouveau palier de statut app ("en cours de tri") pour cette v1, perte de précision assumée plutôt que d'étendre l'app mobile dans cette passe. Rétrocompatible : sur un entrepôt resté à 1 étape, comportement strictement identique à avant.
- **Regroupement de commandes en lot** : `stock.picking.batch` (standard), appliqué à l'étape Pick seulement. **Suivi par commande au poste de tri** : `stock.quant.package` (standard, mécanisme "Mettre en colis") — un bac/colis par commande. Aucun champ standard ne relie `stock.quant.package` à `sale.order` : **convention de nommage** (`package.name = order.name`), pas de nouveau champ custom.
- **Calcul du regroupement optimal — seule partie sans équivalent standard, custom justifié** : Odoo ne sait grouper automatiquement que par critères simples (même jour, même type d'opération) — aucune notion de similarité produit ou de contrainte de capacité multi-critères. Nouveau modèle `x_batch_picking_wizard` (`TransientModel`, action de type **tableau de bord** — calcule sur l'ensemble des commandes `sale` en attente, pas sur un enregistrement précis via `active_id` comme `x_pin_reset_wizard`) + `x_batch_picking_wizard_line` (une ligne par commande candidate, numéro de lot éditable par l'opérateur avant validation — garder un humain dans la boucle, pas d'automatisation totale silencieuse pour cette v1). Moteur de clustering isolé en pur Python (`models/batch_picking_engine.py`, aucune dépendance ORM, testé directement en sandbox faute de Docker) : algorithme glouton — fenêtre de temps compatible en amont (candidates = commandes déjà confirmées, `state == 'sale'`, étape Pick pas encore batchée), puis par bucket : graine = commande à similarité totale maximale avec le pool restant (recalculée à chaque tour, pas figée), croissance gloutonne par similarité avec l'ensemble cumulé du lot (Jaccard sur `product_tmpl_id`, calcul incrémental), sous 3 contraintes dures configurables (nombre de bacs au poste de tri, charge opérateur = quantité cumulée, temps de traitement au tri = nombre de lignes cumulé) et un seuil minimal de similarité (sinon le glouton regrouperait des commandes sans aucun rapport). Règle fair-play : une commande qui dépasse un seuil SLA configurable devient graine forcée du prochain lot, indépendamment de son score (override dur, plus explicable pour l'opérateur qu'un score composite pondéré). Tie-break déterministe (ancienneté puis id) — indispensable puisqu'un humain compare des suggestions d'un clic à l'autre.
- **Paramètres réglables** (`ir.config_parameter`, pas de nouveau modèle pour quelques scalaires — `data/batch_picking_data.xml`) : `echango_order.batch_max_orders` (bacs, défaut 6), `batch_max_qty` (charge opérateur, défaut 100), `batch_max_lines` (plafond tri, défaut 40), `batch_min_similarity` (défaut 0.10), `batch_sla_hours` (fair-play, défaut 4h) — valeurs par défaut arbitraires, à calibrer avec des données réelles (points ouverts, non tranchés avec l'utilisateur pour cette v1 — "ajuster plus tard"). **Réglables depuis l'UI** (retour utilisateur après premier test réel : le mode développeur/Paramètres système était peu accessible) : `res.config.settings` étendu (`models/res_config_settings.py`, champs `x_batch_*` avec `config_parameter=...`, mécanisme standard — aucun code de lecture/écriture custom), formulaire dédié sous un nouveau menu "Paramètres de préparation groupée" (Echango Order), volontairement en dehors du formulaire général de l'app Réglages. Point de vigilance non vérifié en réel : `res.config.settings` est par défaut réservé au groupe Réglages (`base.group_system`) côté Odoo standard — à confirmer qu'un opérateur sans ce groupe voit bien le menu, sinon lui accorder ce groupe ou revoir l'accès.
- **Hors périmètre v1** (rappel explicite) : pas d'app préparateur mobile, pas d'automatisation totale sans validation humaine, chaîne du froid/frais-surgelé (catalogue actuel sans cette contrainte, à revisiter si le catalogue évolue), gestion des ruptures de stock découvertes en cours de tournée sur un lot déjà en cours (non pensé, lien possible avec F17 à concevoir plus tard).

## Custom fields Odoo attendus (Expert Odoo)

`x_reception_mode`, `x_creneau`, `x_firebase_token`, `x_vitrine_publique`, `x_pin` (hashé, sur `res.users`), `x_adresse_favorite` (booléen sur `res.partner`, implémenté F10 — voir ci-dessus), `x_verification_state` (Selection sur `res.partner`, implémenté — voir § Qualité clients ci-dessus), `x_last_activity` (Datetime sur `res.users`, implémenté — session 24h d'inactivité, voir § Principe architecture Odoo ci-dessus), modèle `x_delivery_zone`, modèle `x_rate_limit` (compteur anti-abus à fenêtre fixe par IP, implémenté — endpoints publics rate limités, voir § Sécurité/`controllers/rate_limit.py`), modèle `x_timeslot_capacity` (capacité par créneau/mode de réception, implémenté — voir ci-dessus), modèle `x_pin_reset_wizard` (assistant de réinitialisation du PIN par un modérateur, implémenté — voir ci-dessus), modèle `x_substitute_product_ids` (produits de substitution curés par l'admin, implémenté — voir § Produits de substitution ci-dessus), `x_delivery_status` (Selection sur `sale.order`, implémenté — statut de livraison à domicile déclaré manuellement, voir § Statuts de commande ci-dessus), `x_pickup_collected` (Boolean sur `sale.order`, implémenté — statut de retrait magasin déclaré manuellement, voir § Statuts de commande ci-dessus), modèle `x_batch_picking_wizard`/`x_batch_picking_wizard_line` (assistant de suggestion de lots de préparation groupée, implémenté — voir § Préparation groupée ci-dessus).
