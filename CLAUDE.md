...
- **2e `stock.picking.batch` créé automatiquement pour l'étape Pack** (trouvaille en test réel, 2026-07-22) : ni Batch ni Wave Transfers ne relient nativement le lot de collecte à ses transferts Pack correspondants (Wave ne mélange jamais deux types d'opération) — sans ce 2e lot, l'opérateur de tri devait retrouver chaque commande une par une via la commande elle-même, perdant l'intérêt du regroupement. `action_create_batches()` (`batch_picking_wizard.py._pack_pickings()`) résout les transferts Pack via `order.warehouse_id.pack_type_id` (même schéma que `pick_type_id`) et les regroupe dans un lot distinct, nommé "Tri — lot N" (vs "Collecte — lot N" pour le Pick) — toujours `stock.picking.batch` standard, pas de nouveau modèle.
