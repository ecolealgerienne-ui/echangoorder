...
- `stock.picking.batch_id` n'existe **pas** dans le module `stock` de base — module séparé `stock_picking_batch` (Community, LGPL-3), à déclarer explicitement en dépendance.
- `stock.quant.package` renommé `stock.package` en Odoo 19 (confirmé contre le code source) — mêmes champs (`name` requis), toujours dans le module `stock` de base.
- Ni Batch ni Wave Transfers ne relient deux étapes différentes (Pick/Pack) d'un même regroupement — Wave ne mélange jamais deux types d'opération (confirmé contre la doc Odoo officielle). `stock.warehouse.pack_type_id` (même schéma que `pick_type_id`) permet de résoudre le transfert Pack d'une commande sans passer par elle.
...
2. **En attente de prise en charge** — le client confirme dans l'app ; **les détails (adresse/créneau) restent modifiables** via `checkout_controller.py.confirm()`, rappelable tant que l'état est `draft`/`sent`. Réutilise `state == 'sent'` (valeur standard, relabellée `selection_add`) — `checkout_controller.py.confirm()` écrit `state='sent'` directement (pas `action_quotation_sent()`, pensé pour l'email manuel). **Le panier (articles), lui, ne considère plus une commande `sent` comme "courante"** (décision produit 2026-07-23, rupture nette — revient sur un essai précédent qui incluait `sent` dans `cart_controller.py._cart_order()`/`_owned_line()` : un ajout après confirmation s'attachait alors silencieusement à la commande déjà confirmée, invisible côté app tant qu'un opérateur ne l'avait pas prise en charge). Un nouvel ajout après confirmation démarre systématiquement une commande `draft` neuve.
...
- **Navigation Pick → Pack : décision produit (2026-07-22) de ne rien coder** — options écartées après discussion : un 2e `stock.picking.batch` automatique pour le Pack (essayé puis retiré, complexité jugée pas prioritaire) et le module OCA `stock_picking_show_linked` (porté en 19.0, AGPL-3, mais dépendance externe à déployer pour un gain jugé secondaire). Retenu : recherche standard (Inventaire > Transferts, filtre "Document d'origine" = référence de la commande) — `stock.picking.origin` est un simple texte, pas cliquable (vérifié en test réel), mais la recherche reste rapide. Voir `docs/specs_preparation_groupee.md` pour le détail des options comparées.
...
- **Scan code-barres, exigence confirmée pour l'app préparateur (futur)** : éliminer les erreurs de frappe au Pick et au Pack. Vérifié (2026-07-22) : ni l'app Barcode native d'Odoo (`stock_barcode`, Enterprise uniquement, absente du dépôt Community) ni son alternative OCA (`stock_barcodes`, arrêtée à la version 16.0, pas portée en 19) ne conviennent pour Odoo 19 Community. Le scan sera construit dans l'app préparateur elle-même (caméra du téléphone ou douchette dédiée côté Flutter), pas via Odoo — voir `docs/specs_app_preparateur.md` pour la conception complète (architecture, précédent OCA Shopfloor/Camptocamp Moove étudié, 3 revues spécialisées).
- **Point de vigilance trouvé en concevant l'app préparateur, à corriger dans le code existant** : `batch_picking_wizard.py._candidate_orders()` prend le premier picking Pick trouvé (`[:1]`) pour une commande — un reliquat (backorder) sur le Pick (rupture partielle en cours de collecte, déjà rencontré) crée un 2e picking Pick pour la même commande, cas que ce pattern ne gère pas correctement. Pas corrigé à ce jour (trouvé en revue, pas en test réel) — à traiter avant un usage intensif du wizard.
