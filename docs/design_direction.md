...
- **Contraste** : vérification WCAG (ratio de luminance) sur les paires texte/fond clés des deux thèmes — toutes ≥ 4.5:1 sauf une : le texte blanc des badges "Épuisé"/"Promo" (11px, gras) sur fond rouge/terre cuite atteint ~4.4:1, sous le seuil AA pour du texte de cette taille (seuil atteint uniquement pour du texte "large"). **Pré-existant, non introduit par ce chantier** (couleurs badge inchangées depuis avant la Phase 1) — non corrigé ici pour éviter d'élargir la portée à un token partagé (`AppColors.danger`/`promo`) utilisé ailleurs avec un contraste correct (boutons, icônes) ; à revisiter si un audit accessibilité dédié est demandé.
